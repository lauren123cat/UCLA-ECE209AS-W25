...
class solution:
    ...
    def getPassword(self):
        ...
        # infinite while loop until it finds the correct pw
        while True: 
            for char in all_chars:
                curr_pass = curr_correct + char
                self.setProtectMem(len(curr_pass), len(curr_pass))
                res = self.checkPassword(curr_pass)
...
Start with an empty string and append all possible chars to it.
Set memory protection at length of found password.
    If wrong, guessed char is incorrect -> try another one
    If seg fault, guessed char is correct 
    If correct, the password is found!
(Repeat until it's found)
"""